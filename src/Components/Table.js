...
export default function EnhancedTable() {
  ...
  React.useEffect(() => {
    const getData = async () => {
      await fetch(
        `http://localhost:8081/backend_hrc_final/GetUser?page=${page}&rowsPerPage=${rowsPerPage}`
      )
        .then((res) => res.json())
        .then((data) => {
          console.log(data);
  ...
  const handleClick = (event, sl_no) => {
    ...
    if (newSelected.length === 1) {
      ...
      setEdit({
        invoice_currency: editvar[0].invoice_currency,
        cust_payment_terms: editvar[0].cust_payment_terms,
      });
      console.log(editvar);
    }
  };
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%" }} style={{ background: "#283a46" }}>
        <Form
          enable={enable}
